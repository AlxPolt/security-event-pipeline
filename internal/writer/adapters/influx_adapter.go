...
import (
	...
	"github.com/AlxPolt/security-event-pipeline/pkg/logger"
	"github.com/AlxPolt/security-event-pipeline/pkg/sanitizer"
	"github.com/AlxPolt/security-event-pipeline/pkg/security"
)
...
func (r *InfluxRepository) Save(ctx context.Context, event domain.Event) error {
	...
	point := influxdb3.NewPoint(
		...
		map[string]string{
			...
			// event_id disambiguates points that would otherwise collide on
			// InfluxDB's point-identity dedup (measurement + tag set +
			// timestamp): two distinct events with the same severity landing
			// on the same nanosecond — plausible with multiple daemon
			// replicas — would silently overwrite one another without it.
			// A redelivered NATS message carries the same ID, so retries
			// still dedup onto the same point as intended.
			"event_id": event.ID,
		},
...
func (r *InfluxRepository) ensureDatabase(ctx context.Context) error {
	...
	if resp.StatusCode == http.StatusUnauthorized || resp.StatusCode == http.StatusForbidden {
		return fmt.Errorf("listing databases failed with status %d: INFLUX_TOKEN is invalid or lacks permission", resp.StatusCode)
	}
...
// verifyConnection issues a probe query and distinguishes "table doesn't
// exist yet" (expected on first boot, before any event has been written)
// from a real connectivity/auth problem. Auth failures are checked via the
// gRPC status code first — an Unauthenticated/PermissionDenied response must
// never be swallowed, even if its message happens to mention "table" (e.g.
// "permission denied for table events_tab"), which a bare substring match on
// "table" would incorrectly treat as success.
func verifyConnection(client *influxdb3.Client, database string) error {
	...
	if st, ok := status.FromError(err); ok {
		...
		case codes.Unauthenticated, codes.PermissionDenied:
			return fmt.Errorf("verifying InfluxDB connection: authentication failed: %w", err)
		case codes.NotFound:
			return nil
		}
	}
	...
	if errStr == "no data" ||
		...
		strings.Contains(errStr, "table 'events_tab' not found") ||
		strings.Contains(errStr, "table \"events_tab\" not found") {
		return nil
	}
	return err
