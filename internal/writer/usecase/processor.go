...
import (
	...
	"github.com/AlxPolt/security-event-pipeline/internal/writer/ports"
)
...
// produces no extra data — the event carries a stable ID (assigned once at
// creation, see internal/daemon/domain.Generator.NewEvent) as a tag, so a
// retried/redelivered message overwrites its own prior write rather than
// colliding with an unrelated event that happens to share a severity and
// timestamp.
