...
Although the payload here is security events, the pipeline's shape is a general pattern for ingesting from many distributed, unreliable sources: durable at-least-once delivery from source to storage, backpressure so a burst of producers can't overwhelm consumers, horizontal scaling of stateless consumers behind a shared durable stream, and point-identity design in the time-series store so retries and replays don't corrupt data (see [Point Identity & the `event_id` Tag](#point-identity--the-event_id-tag)). The same shape applies directly to telemetry from fleets of distributed IoT/edge devices — e.g. metering, sensor, or energy-asset data — where devices publish independently, delivery can't be guaranteed exactly-once, and the write path must scale horizontally with device count.
...
`id` is a UUID v4 assigned once, at generation time, and carried unchanged through NATS and into InfluxDB (see below).
...
##### Point Identity & the `event_id` Tag

InfluxDB v3 deduplicates points that share the same measurement, tag set, and timestamp — the last write wins. Tagging points only by `severity` is not enough: two *distinct* events with the same severity landing on the same nanosecond (plausible with multiple Writer/Daemon replicas, see [Horizontal Scaling](#horizontal-scaling)) would silently overwrite one another, despite the at-least-once ACK machinery reporting success.

Each event carries a UUID `id`, assigned once at generation time and written through to InfluxDB as an `event_id` tag. This gives point identity two properties at once:

- **Distinct events never collide** — different events get different IDs, even at identical timestamps.
- **Redelivery stays idempotent** — a message redelivered by JetStream (after a NAK or worker crash) carries the *same* ID, so the retried write overwrites its own prior attempt rather than creating a duplicate row.
...
|---------|-------------|-----------|
| Writer exits immediately with `authentication failed` / `INFLUX_TOKEN is invalid` | Token mismatch between `INFLUX_TOKEN` and `INFLUXDB3_AUTH_TOKEN` | Ensure both values in `.env` are identical; `make down && make up`. The Writer fails fast at startup on a bad token rather than starting "healthy" and failing on every write. |
